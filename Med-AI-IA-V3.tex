...
different diagnosticians might give different \hautieng{diagnoses}, in which case it is hard to assign ground truth. 
...
over-represent malignant patients and \hautieng{therefore} be biased. If an image-based classifier
...
to be reduced.  However, it does not have to be reduced to a {\em
  single} \hautieng{diagnosis} before treatment can be started.
...
measures such as EKG, EEG, camera images, X-ray, \hautieng{ultrasound} and MRI, some of which might
...
Other types of uncertainty are ``top down'' in that they \hautieng{originate} in medical research percolates down to the medical staff. We briefly
...
{\em Our proposal \hautieng{is to} allow IA to output ``I don't know'' on
...
several doctors \hautieng{is} common sense. A similar approach has been used in
...
We use Ronald Heifetz classification of problems (see \hautieng{inset}). Our
...
mistakes are unavoidable, and the question \hautieng{is} who \hautieng{is} responsible for
...
sleep. Developing such \hautieng{an} automatic system is an
...
reduce the \hautieng{willingness} of sleep experts' usage of such \hautieng{systems}. An ideal IA
...
devices can be placed in the \hautieng{patient's} home and provide the doctor with
vital signs.
...
polysomnogram or \hautieng{ultrasound} currently require a local nurse or a
...
Otherwise, it would output IDK and alert the remote healthcare provider. \hautieng{A similar application to patient compliance is possible. An IA system can help monitor if a patient follows the order or drinks too much remotely, and warn the remote healthcare provider if the compliance is not ideal or if it cannot decide the compliance.}
...
such as protocols, books, lectures and journal articles. \hautieng{It can serve not only as a passive search engine that can offer more relevant information to healthcare providers when they ask for any, but also an active knowledge provider when it senses what healthcare providers need}. They are
particularly suited for training students in pattern areas. An IA can
be used to train and test the \hautieng{student's} ability to accurately and
confidently identify simple patterns, \hautieng{and leave it} to senior and
