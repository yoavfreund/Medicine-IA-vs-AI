...
\input{Arrhythmia}
One approach for quantifying uncertainty in medical diagnosis is to use inter-rater studies, where multiple doctors produce diagnostics
based on identical medical information without communicating with each other.
Associating ground truth with these diagnostics whose inter-rater agreement is less than perfect is challenging.
 
In addition, diagnosis is not a simple 
...
a patient that is treated in a clinic.  When a patient
arrives at the clinic for the first time, all diagnostics are
possible. After a physical exam and an interview with a doctor, many
possibilities are eliminated.
\input{InterRaterAgreement}

Cases (patients) differ in their complexity. In {\em simple} cases, the initial exam is enough
for the doctor to confidently choose a treatment. In a more complex
case, the doctor might ask for multiple tests and visits, refer the
patient to a specialist, consult colleagues, journals and books,
etc, to narrow the set of likely diagnostics and choose a treatment plan. 
However, it does not have to be reduced to a {\em  single} diagnosis before treatment can be started.
In order for IA to aid the HP in the diagnostic process it has to have a way to express both what is knows and what it {\em does not} know.
...
adherence, changes in living and working environments, diet, stress, etc.

Given the difficulties in measuring an undisputed "ground truth" for diagnostics, we suggest that the goal of learning be set not to 
predicting a single "true" diagnostic, but rather to taking the actual diagnostics given by the HP as the ground truth. In other words, we change the goal from performing better than the human to that of performing almost as well as the humans. Which means that uncertainty or disagreement among the HP should be reflected as uncertainty in the output of the IA helper, or, in other words, by IDK. This conforms to the goals of AI vs IA, while AI's goal is to replace the HP, the goal of IA is to aid the HP and make him/her more efficient and more accurate.

\input{SignalQuality}
There are many causes for uncertainty in human medical diagnosis. We briefly
