...
The meteoric rise of AI \hautieng{(Artificial Intelligence)} and Deep learning raises the possibility that
...
\input{AIIABox} The argument as to whether human healthcare providers (HP)~\footnote{Our discussion applies to all healthcare providers, including Doctors, Nurse Practitioners and Nurses. To simplify the references \hautieng{to} this group of professionals we use the acronym HP, which stands for "Healthcare Provider"}  will be replaced or empowered by computers is a recent
incarnation of a long standing debate between \hautieng{AI}  and IA
(Intelligence amplification) \hautieng{\sout{(see inset)}}.
...
\hautieng{its} own limitations, will give advice only when the evidence is
strong and otherwise say \hautieng{IDK \sout{``I don't know''}}.
...
Deep learning (DNN) is a special case of {\em supervised learning} \hautieng{\sout{(see
...
makes the claim that DNNs can perform diagnostics as well as, or better, than board certified dermatologists. \hautieng{\sout{(see inset)}}
...
based on identical medical information without communicating with each other. \hautieng{\sout{(see inset)}}.
Associating {\em ground \hautieng{truth}} with these diagnostics whose inter-rater agreement is less than perfect is challenging.
...
measures such as \hautieng{electrocardiogram, electroencephalogram}, camera images, X-ray, ultrasound and MRI, some of which might
...
called ``alarm fatigue'' \hautieng{\sout{(see inset)}} where the medical staff ignores
...
tool, \hautieng{they might not be always available. \sout{they have some limitations, as described in the inset.}}
~\\
...
diseases with high \hautieng{\sout{rates of}} inter-rater disagreement \hautieng{rate} is
...
doctors tend to either say \hautieng{IDK \sout{"I don't know"}} or disagree with each other.
...
dominant majority rule outputs ``?'' which is interpreted as \hautieng{IDK \sout{``I don't
know''}}.
...
We use Ronald Heifetz classification of problems \hautieng{\sout{(see inset)}}. Our
...
caregiver. Naturally, \hautieng{HP \sout{the caregiver}} will prefer not to use the
system. Will the patient prefer the human \hautieng{HP \sout{the caregiver}} or the AI system?
The answer is anybody's guess, because such systems are not available
yet. Studies show that patients trust a human \hautieng{HP \sout{the caregiver}} more than
technology \cite{ongena2020patients}, and that trust requires a human
to human connection \cite{nundy2019promoting}.

Unlike AI, IA systems do not aim to replace \hautieng{HP \sout{the caregiver}}. Rather,
they aim to assist \hautieng{HP \sout{the caregiver}} by taking care of the simple or easy
...
This makes \hautieng{HP \sout{the caregiver}} more efficient and effective, but does
not take away the agency and humanity of \hautieng{HP \sout{the caregiver}}. When the IA system outputs
\hautieng{IDK \sout{``I don't know''}} it explicitly passes the responsibility for the
patient to \hautieng{HP \sout{the caregiver}}.

%Agency and responsibility are central to patient-caregiver
%relationship.

As discussed earlier, \hautieng{challenging} medical cases often fall within gray areas, where
medics differ on the correct diagnosis or the best treatment. In such
cases \hautieng{HP \sout{the caregiver}} has the responsibility of making a decision even
...
well-being is {\em always} with the human \hautieng{HP \sout{the caregiver}}. Even if the AI
system is known to make fewer mistakes than the average doctor \hautieng{in some well-defined tasks},
mistakes are unavoidable, and the question is who is \hautieng{\sout{held}} responsible for
...
of recognition \hautieng{when the task is about recognizing patterns}. Their experience allows them to quickly sift through
large amounts of data and detect complex patterns. \hautieng{\sout{Pattern}} Doctors \hautieng{handling patterns}
...
Pattern doctors are often unable to fully explain \hautieng{challenging \sout{their}} diagnostics.
...
\cite{sleepHT2020}.
There is \hautieng{non-trivial \sout{significant}} inter-rater agreement rate among experts\hautieng{, even} for 
{\em normal subjects}\hautieng{\sout{, it is still far from perfect}}. Agreement drops significantly for abnormal subjects with \hautieng{\sout{subjects with}} sleep apnea. 
...
An ideal IA
sidekick should let experts know \hautieng{IDK \sout{"I don't know"}}, where there is any
ambiguity in the process of annotation. Suppose there are 10\% \hautieng{IDK \sout{"I
...
move\hautieng{, an} approach to long term care called ``aging in
...
medic using them. This is particularly true for \hautieng{\sout{pattern}} doctors \hautieng{that
perform pattern} analysis of complex signals. Initially, the sidekick will
...
which are often hard to describe in words. \hautieng{\sout{Training} Implementing} a cancer detection model \hautieng{for the training purpose} is likely to require less effort and be more effective than writing a \hautieng{tutorial \sout{journal article}} that explains the same \cite{reid2000medical}.

These models are likely to agree with each other on the easy cases, but are likely to disagree on the harder cases. By comparing the outputs of models trained by multiple experts \hautieng{one} can distinguish the easy cases, on which a confident prediction can be made, from the harder cases, on which the diagnosis is IDK, and the doctor needs to use additional tests to arrive at a reliable diagnosis.
