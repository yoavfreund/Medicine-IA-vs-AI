...
{\color{blue}To appreciate the ``black box'' issue, let us look at this example.}
...
{\color{blue}Decision making in clinics is not only a science, but also an art. It is a science since it should be based on a the best available knowledge and evidence about the individual patients \cite{}. This scientific approach is propagandized as the ``evidence-based medicine'' \cite{}. Based on the evidence, medical guidelines and protocols could be established. The healthcare personnels could make better medical decisions based on the protocols and hence provide a better quality of medical care. However, decision making is still an art in many scenarios. Human body is complicated, and it is clearly that unless we have an access to its design book, the protocol we establish could have gaps. When facing these gaps, physicians will make the decision based on the available information, his experience, or even make a guess. As a result, while} in some cases, physicians can achieve a reliable
