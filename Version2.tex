...
\yoav{I think getting this right is important}

\sout{an objectively measurable fact} {\color{red}always
  perfect}; \yoav{an incontrovertible fact?}


rather, it represents the judgment of human diagnostician, and
different diagnosticians can have differ in their diagnosis. We will
...
necessary will be biopsied. {\color{blue}Examples of this kind include arrhythmia classification from single channel electrocardiogram \cite{hannun2019cardiologist}, diabetic retinopathy detection from retinal fundus photographs \cite{gulshan2016development}, etc.}
...
diagnostics. One evidence for this difficulty is that supporting {\em inter-rater
  disagreement} is often high.
\sout{\color{red}[Hau-Tieng: This sentence reads strange here]} (see
inset). In studies of this kind, multiple doctors produce diagnostics
based on identical medical information without communicating with each other. \input{Arrhythmia}
...
\yoav{The next section needs to be resolved}
...
the {\em best} treatment plan, since we can only use a single
...
cases, {\color{blue}or achieve the goal ``computer and human work together'' \cite{verghese2018computer}.} This makes the caregiver more efficient and effective, but does
...
future, medicine will not be a precise science and incorrect decisions
...
Medicine in the 21'st century is highly centralized. Most interaction
...
signs such as blood pressure, heart rate and temperature. More complex
signals such as EKG or ultra-sound currently require a local nurse or a
technician.
...
in many institutions, and be collected in repositories. Such models
...
patterns.  Using an IA model can overcome protocol limitations such as
