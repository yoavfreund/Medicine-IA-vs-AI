...
IA is a more realistic, and ultimately more beneficial approach. {\color{blue}We mention that
this seemingly restricted IA is considered by American Medicine Association and other medical professional associations as the currently favorable framework, which they called ``augmented intelligence'' \cite{american2019augmented,AAD2019augmented}.}
...
judge the additional{\color{blue}, sometimes invasive,} test to be necessary, in which case it will not
...
as heart rate, blood pressure and temperature can be measured {\color{blue}more} reliably
and {\color{blue}consistently than other}
devices such as EKG, EEG, camera images, X-ray, ultra-sound and MRI, {\color{blue}which might sometimes}
...
\input{SignalQuality} Signal quality enhancement is already an
...
\input{AlarmFatigue}
  One situation where signal quality and signal variability {\color{blue}cannot be neglected} is Patient Monitors. The purpose of these
...
the generated alarms, rendering them {\color{blue}less clinically useful as expected}.
