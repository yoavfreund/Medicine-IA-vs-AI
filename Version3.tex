...
\InsertBoxR{0}{
  ...
  \fcolorbox{DeepSkyBlue1}{Azure1!30}{\begin{minipage}{2.5in}
...
\fi
  ...
  \Medicine{Bladder and sphincster diagnosis}{ Urodynamic studies
    provide the best bladder and sphincter functional data for
    urologists to decide how to treat patients at risk for renal
    damage \cite{abrams2003describing}.  While it has been extensively
    studied and applied in clinics, the main issue that plagues this
    field of urodynamics is the lack of precise definition of a
    detrusor contraction or overactive contraction
    \cite{abrams2003describing}.
%  
    The lack of a well defined definition is due to the lack of
    quantitative study from the pathophysiological perspective, so the
    definition is still based on ``expert opinion''. For example,
    usually an overactive contraction represents itself as a ``bump''
    in the detrusor pressure signal. However, what is the breath and
    height of a bump should we call it an overactive contraction? How
    to distinguish a true overactive contraction from an artifact?
    While there have been several reference information, like
    abdominal pressure, that could help us identify artifacts, but it
    can only explain a small portion of them.
...
\Medicine{Recognition vs. Elimination in Medical diagnosis}{In general, diagnosis is
  performed by comparing observed symptoms to past experience. Roughly
  speaking there are two ways to make this comparison: {\em
    recognition} and {\em elimination}.  Recognition is a fast,
  typically non-verbal, mental process in which the doctor identifies
  a pattern in the symptoms and instinctively makes a diagnosis. On
  the other hand, elimination is a slow deliberative process through
  which the doctor methodically eliminates diagnostic possibilities.

  Recognition is a faster than elimination, but is more error
  prone. Recognition tends to ``lock on'' to a aparticular diagnosis and
  ignore aternative diagnoses. Elimination is a processes that starts
  with all possible diagnoses and iteratively reduces them. Ideally
  this process of elimination ends up with a single
  diagnosis. However, there might be more than one diagnosis left. In
  that case, additional test might be needed to eliminate all but one
  of the diagnosis.
 }
...
\section*{The scarcity of ground truth}

Supervised machine learning is a voracious consumer of {\bf labeled
  data}~(\cite{topol} page XXX). Unfortunately, in most medical
applications  of machine learning, labeled data, also called ``ground
truth'' is not available. 
\InsertBoxR{0}{
  \footnotesize\setlength\fboxsep{10pt}\setlength\fboxrule{1pt}
  \fcolorbox{DarkOrchid2}{DarkOrchid2!10}{\begin{minipage}{3.5in} % ML 
      \invisiblesection{\textit{Side Bar}}
      \subsubsection*{Skin cancer diagnosis using Deep Neural Network}
      One of the
      papers that provided evidence that deep neural networks might be
      able to outperform humans is the work of Esteva et
      al~\cite{esteva2017dermatologist}. They trained a Deep neural
...
\InsertBoxR{3}{
  \footnotesize\setlength\fboxsep{10pt}\setlength\fboxrule{1pt}
  \fcolorbox{IndianRed3}{AntiqueWhite1!30}{\begin{minipage}{3.5in}
      \invisiblesection{\textit{Side Bar}}
      \subsubsection*{Supervised Learning and ground truth}
      {Roughly
...
% should me moved to uncertainty in ML
...
\InsertBoxR{0}{
  \footnotesize\setlength\fboxsep{10pt}\setlength\fboxrule{1pt}
  \fcolorbox{DeepSkyBlue1}{Azure1!30}{\begin{minipage}{2.5in} %ORG
% \fcolorbox{DarkOrchid2}{DarkOrchid2!30}{\begin{minipage}{3.5in} % ML 
% \fcolorbox{IndianRed3}{AntiqueWhite1!30}{\begin{minipage}{3.5in} % MED
      \invisiblesection{\textit{Side Bar}}
      \subsubsection*{AI vs IA}
      Using computers to augment human intelligence rather replace it is
    \end{minipage}}
